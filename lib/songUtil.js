var fs = require('fs');
var scales = JSON.parse(fs.readFileSync('./node_modules/midi-generator/lib/scales.json', 'utf8'));
class Pitch {
...
class Motif {
  ...
  static drumPat(scoreInfo, instData, args){
    ...
    for(var i = 0, unit = scoreInfo.unitLen / density; i < density; i++){
      //hihat
      var hihatAction = Math.random();

      if(hihatAction > 0.9){
        timing.push(i * unit);
        dur.push(unit * 0.9);
        pitch.push(hihat);
        var double = (i * unit) + (unit/2);
        timing.push(double);
        dur.push((unit/2) * 0.9);
        pitch.push(hihat);
      } else if (hihatAction > 0.8){
        timing.push(i * unit);
        dur.push(unit * 0.9);
        pitch.push(hihat);
        var eighth = unit * 2;
        var triplet = eighth / 3;
        var double = (i * unit) + (triplet * 1);
        timing.push(double);
        dur.push(triplet * 0.9);
        pitch.push(hihat);
        var triple = (i * unit) + (triplet * 2);
        timing.push(triple);
        dur.push(triplet * 0.9);
        pitch.push(hihat);
      }
        else if (hihatAction > 0.6){
